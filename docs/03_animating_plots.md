...
You can use [`g3p`](./api_ref.md#namespace_g3p)`::`[`version`](./api_ref.md#gnuplot_version)`()` function to switch between terminals based on the <wiki:Gnuplot> version:
...
(g3p_display)=
## Using [`g3p`](./api_ref.md#namespace_g3p)`::`[`display`](./api_ref.md#display_func) in a loop

[`g3p`](./api_ref.md#namespace_g3p)`::`[`display`](./api_ref.md#display_func) function has an optional 2{sup}`nd` argument for choosing if the previous output should be cleared or not. It's on by default, meaning if we don't provide it, the new plot will replace the previous one. We can use this feature to create a dynamic animation effect. There are two downsides for this approach:
...
That being said, here's the same sine wave animation implemented using [`g3p`](./api_ref.md#namespace_g3p)`::`[`display`](./api_ref.md#display_func) in a loop:
