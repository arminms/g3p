...
Yet another way to explicitly display a plot is `g3p::display()` function. This one has an optional 2{sup}`nd` argument that can be used to [show an animated plot](#g3p_display).
::::
